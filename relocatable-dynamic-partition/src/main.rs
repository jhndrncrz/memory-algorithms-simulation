...
fn main() {
    ...
    for snapshot in &system.snapshots {
        if snapshot.number != system.memory.set_counter {
...
fn input_jobs(is_last_input_valid: &mut bool) -> Vec<Job> {
    ...
    for i in 0..jobs_count {
        ...
        loop {
            ...
            job_size_input = display_prompt(
                "Enter",
                &format!(
                    "memory size of {} (in M)",
                    text_apply_style(&format!("Job {}", i + 1), "fg-yellow")
                ),
            );
        ...
        loop {
            ...
            job_turnaround_time_input = display_prompt(
                "Enter",
                &format!(
                    "turnaround time of {} (in M)",
                    text_apply_style(&format!("Job {}", i + 1), "fg-yellow")
                ),
            );
...
impl System {
    ...
    fn handle_jobs(self: &mut Self) {
        ...
        while !self.main_queue.is_empty() || jobs_count != 0 {
            if !self.main_queue.is_empty() {
                ...
                match self.memory.allocate(current_job) {
                    ...
                    Err(returned_job) => match returned_job.status {
                        JobStatus::Waiting => {
            ...
            if self.main_queue.is_empty() {
                ...
                let set: Set;
                let mut deallocating_set: Set;
                let deallocated_set: Set;
                let relocated_set: Set;

                set = self.memory.take_set();
                deallocating_set = self.memory.take_set();

                match self.memory.deallocate() {
                    Ok((mut unloaded_jobs, set)) => {
                        deallocated_set = set;
                        while !unloaded_jobs.is_empty() {
                            ...
                            for index in 0..deallocating_set.jobs_name.len() {
                                if unloaded_job.name == deallocating_set.jobs_name[index] {
                                    deallocating_set.jobs_name[index] =
                                        format!("[{}]", unloaded_job.name);
                                }
                            }
                ...
                snapshot.log.push(Log::Relocating);
                self.memory.finalize_partitions();
                relocated_set = self.memory.take_set();
                self.memory
                    .delete_partition(self.memory.partitions.len() - 1);

                snapshot.log.push(Log::Requeuing);
                while !self.waiting_queue.is_empty() {
                    self.main_queue
                        .push_front(self.waiting_queue.pop_back().unwrap());
                }

                snapshot.set = set;
                snapshot.set_deallocating = deallocating_set;
                snapshot.set_deallocated = deallocated_set;
                snapshot.set_relocated = relocated_set;
        ...
        snapshot.set_deallocating = self.memory.take_set();
        snapshot.set_deallocated = self.memory.take_set();
        snapshot.set_relocated = self.memory.take_set();
        self.snapshots.push(snapshot);
    }
    ...
    fn display_snapshots_summary(self: &Self) {
        ...
        for index in 0..longest_set_length {
            partitions_name.push(
                self.snapshots[longest_set_index]
                    .set_deallocating
                    .partitions_name[index]
                    .clone(),
            );
        }
        ...
        for snapshot in &self.snapshots {
            ...
            for index in 0..longest_set_length {
                if index >= snapshot.set_deallocating.partitions_name.len() {
                    set.push("".to_string());
                } else {
                    set.push(format!(
                        "{}: {}",
                        snapshot.set_deallocating.jobs_name[index],
                        snapshot.set_deallocating.jobs_size[index]
                    ));
                }
            }
    ...
    fn display_conclusion(self: &mut Self) {
        ...
        let partitions_count: usize;
        (_, partitions_count) = self.find_longest_set();

        display(&format!(
            "The number of partitions created is at most {}.\n",
            partitions_count - 1
        ));
        display(&format!(
...
impl Snapshot {
    fn new(number: usize) -> Self {
        return Self {
            ...
            log: Vec::new(),
            set: Set::new(),
            set_deallocating: Set::new(),
            set_deallocated: Set::new(),
            set_relocated: Set::new(),
        };
    }
    ...
    fn display_snapshot(self: &Self) {
            ...
            match entry {
                Log::Loaded(job_name) => {
                    display_success(&format!(
                        "Memory for {} is allocated successfully.\n",
                        job_name
                    ));
                }
                Log::Unloaded(job_name) => {
                    display_success(&format!(
                        "Memory for {} is deallocated succesfully.\n",
                        job_name
                    ));
                }
                ...
                Log::Relocating => {
                    display_divider(1);
                    display_log("Relocating memory for jobs and compacting free memory...\n");
                }
                Log::Deallocating => {
                    display_divider(1);
        ...
        display_table(&heading, &self.set.tabularize());
        ...
        display_title(&format!("Set {}\n", self.number));
        display_subtitle("After Allocation\n");
        display_divider(1);
        display_table(&heading, &self.set_deallocating.tabularize());
        ...
        display_title(&format!("Set {}\n", self.number));
        display_subtitle("After Deallocation\n");
        display_divider(1);
        display_table(&heading, &self.set_deallocated.tabularize());
        ...
        display_title(&format!("Set {}\n", self.number));
        display_subtitle("After Relocation\n");
        display_divider(1);
        display_table(&heading, &self.set_relocated.tabularize());
...
impl Set {
    ...
    fn tabularize(self: &Self) -> Vec<Vec<String>> {
        let mut body: Vec<Vec<String>>;
        body = Vec::new();
        ...
        for index in 0..self.partitions_name.len() {
            body[0].push(self.partitions_name[index].clone());
            body[1].push(self.partitions_capacity[index].clone());
            body[2].push(format!(
                "{}: {}",
                self.jobs_name[index], self.jobs_size[index]
            ));
        }
...
impl Memory {
    ...
    fn create_partition(self: &mut Self, partition_size: f64) {
        if partition_size <= self.free_capacity {
            self.partitions
                .push(Partition::new(self.partitions.len(), partition_size));
            self.free_capacity -= partition_size;
        }
    }

    fn delete_partition(self: &mut Self, index: usize) {
        if self.partitions[index].is_empty() {
            self.free_capacity += self.partitions[index].capacity;
            self.partitions.remove(index);
        }
    }

    fn relocate(self: &mut Self) {
        for index in 0..self.partitions.len() {
            if index == 0 {
                continue;
            }

            self.partitions[index].number = index;
            self.partitions[index].name = format!("Partition {}", index);
        }
    }
    ...
    fn allocate(self: &mut Self, mut job: Job) -> Result<(), Job> {
        ...
        if self.has_free_capacity(&job) {
            let last_partition_index: usize;
            last_partition_index = self.partitions.len();

            self.create_partition(job.size);

            match self.partitions[last_partition_index].load(job, self.set_counter) {
                Ok(()) => {}
                Err(job) => {
                    return Err(job);
                }
    ...
    fn deallocate(self: &mut Self) -> Result<(Vec<Job>, Set), (Vec<Job>, Set)> {
        ...
        for partition in self.partitions.iter_mut().rev() {
            if let Some(unloaded_job) = partition.job.take() {
                if unloaded_job.set_number.unwrap() + unloaded_job.turnaround_time - 1
                    == self.set_counter
                {
        ...
        if !unloaded_jobs.is_empty() {
            return Ok((unloaded_jobs, set));
        }

        return Err((unloaded_jobs, set));
    }

    fn finalize_partitions(self: &mut Self) {
        if !self.has_complete_partitions() {
            self.partitions
                .push(Partition::new(self.partitions.len(), self.free_capacity));
        ...
        for partition in &self.partitions {
            ...
            match loaded_job {
                ...
                None => {
                    ...
                    loaded_job_size = partition.capacity;
                }
            }

            set.partitions_name.push(partition.name.clone());
            set.partitions_capacity
                .push(format!("{} M", partition.capacity));
    ...
    fn has_free_capacity(self: &Self, job: &Job) -> bool {
        return self.free_capacity >= job.size;
    }
}
