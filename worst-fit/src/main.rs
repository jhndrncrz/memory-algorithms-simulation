...
fn main() {
        ...
        || number_of_allocated_partitions != 0
    {
        ...
        if !jobs_primary_queue.is_empty() {
            ...
            let mut partition_index_largest_unoccupied: usize = 0;
            let mut partition_capacity_largest_unoccupied: f64 = 0.0;
            ...
            for partition_index in 0..number_of_partitions {
                ...
                if partitions_capacity[partition_index] > partition_capacity_largest_unoccupied{
                    partition_index_largest_unoccupied = partition_index;
                    partition_capacity_largest_unoccupied = partitions_capacity[partition_index];
                    job_can_be_allocated_now = true;
                }
            }

            if job_can_be_allocated_now {
                partitions_to_deallocate.insert(0, partition_index_largest_unoccupied);
              
                let partition_name: &String = &partitions_name[partition_index_largest_unoccupied];
                jobs_successfully_allocated[job_index] = true;
                partitions_is_currently_allocated[partition_index_largest_unoccupied] = true;
                partitions_occupier[partition_index_largest_unoccupied] = job_name.clone();
                partitions_occupied[partition_index_largest_unoccupied] = job_size;
                partitions_unoccupied[partition_index_largest_unoccupied] =
                    partitions_capacity[partition_index_largest_unoccupied] - job_size;
...
fn input_partitions(
    ...
    for i in 0..number_of_partitions - 1 {
        loop {
            ...
            display("\n");
            terminal_clear_line(0);
            display_labelled("Available Memory", &format!("{} K", available_capacity));
            terminal_cursor_previous_line(0);
            let partition_capacity_input: String = display_prompt(
                "Enter",
                &format!("memory capacity of Partition {} (in K)", i + 1),
            );
    ...
    display(&format!(
        "Remaining memory ({} K) will be allocated to Partition {}.\n",
...
pub fn display_allocations_bar(
    ...
    io::stdout()
        .write(
            &text_apply_style(
                &format!("{}: {} K / {} K", titles[0], values[0], values[0]),
                "fg-white",
            )
            .as_bytes(),
        )
